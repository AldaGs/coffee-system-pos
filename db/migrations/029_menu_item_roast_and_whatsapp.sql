-- =============================================================================
-- 029_menu_item_roast_and_whatsapp.sql
--
-- Surfaces two per-item fields on the public-menu RPCs so a customer-facing
-- menu can show a coffee's roast date and a "contact / order on WhatsApp"
-- button:
--   * roast_date   ← menu_items.data->>'roastDate'   (ISO yyyy-mm-dd, optional)
--   * whatsapp_url  ← menu_items.data->>'whatsappUrl'  (wa.me / catalog link)
--
-- No table change: both already live in the free-form menu_items.data jsonb
-- (the admin item editor writes them, the menuLocal/menuCloud residual spread
-- round-trips them). This migration only rewrites get_active_menu() and
-- get_menu_by_id() to emit them — verbatim from migration 021 with the two
-- fields added to the per-item jsonb_build_object in each.
--
-- Mirrored in api/install.js and src/components/SetupScreen.jsx.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- Replace get_active_menu — per-item roast_date + whatsapp_url.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Replace get_menu_by_id — same per-item additions.
-- ---------------------------------------------------------------------------
