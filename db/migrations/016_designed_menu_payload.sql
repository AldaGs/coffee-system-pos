-- =============================================================================
-- 016_designed_menu_payload.sql
--
-- Phase 4a (designer templates) needs the catalog payload for kind='designed'
-- menus — templates render from categories + modifier_groups just like the
-- 'live' kind does, but with a layout/theme picked by the owner. Migration
-- 015 returned empty arrays for non-'live' kinds; this replaces
-- get_active_menu so both 'live' and 'designed' get the full payload, while
-- 'pdf' and 'image' still skip it (they render purely from menu.data.pages).
--
...
BEGIN
  ...
  -- 'live' and 'designed' both render from the live catalog. 'pdf'/'image'
  -- render from menu.data.pages and don't need the catalog payload.
