    ...
    <div className="modal-overlay" style={{ zIndex: 100 }}>
      <div className="modal-content" style={{ maxWidth: '500px', background: 'var(--bg-surface)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h2 style={{ margin: 0, color: 'var(--text-main)' }}>{t('edit.title')}</h2>
          <button 
        ...
        <p style={{ color: 'var(--text-muted)', marginBottom: '24px' }}>
          {t('edit.subtitle')} <strong>{editingDrink.drink.name}</strong>.
        </p>
            ...
            return (
              <label 
                ...
                style={{ 
                  ...
                  borderRadius: '8px', 
                  cursor: 'pointer', 
                  background: isAssigned ? 'var(--bg-main)' : 'var(--bg-surface)', 
                  transition: 'all 0.1s' 
                }}
              >
                <input 
                ...
                />
                <span style={{ fontSize: '1.1rem', fontWeight: 'bold', textTransform: 'capitalize', color: 'var(--text-main)' }}>
                  {groupKey.replace('_', ' ')}
                </span>
              </label>
            );
        ...
        <button 
          onClick={() => setEditingDrink(null)} 
          style={{ width: '100%', padding: '16px', background: 'var(--brand-color)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', marginTop: '24px' }}
        >
          {t('edit.btnDone')}
        </button>
      </div>
