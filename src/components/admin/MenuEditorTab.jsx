function MenuEditorTab({ menuData, newCategoryName, setNewCategoryName, handleAddCategory, newItemForm, setNewItemForm, handleAddDrink, handleDeleteCategory, handleDeleteDrink, setEditingDrink, saveMenuToCloud }) {
  return (
    <div>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {menuData.categories[category].length === 0 ? <li style={{ color: 'var(--text-muted)', fontSize: '0.9rem', paddingLeft: '10px' }}>No items yet...</li> : (menuData.categories[category].map(item => (
                  <li key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderBottom: '1px dashed var(--border)', fontSize: '0.95rem', background: 'var(--bg-surface)' }}>
                    ...
                    {/* --- HYBRID INVENTORY TOGGLE (ITEM SPECIFIC) --- */}
                    <div style={{ marginBottom: '16px' }}>
                      <select 
                        value={item.item_type || 'none'} 
                        onChange={(e) => {
                          // Create a copy of the menu, update just this one item, and save it!
                          const updatedMenu = { ...menuData };
                          const catArray = updatedMenu.categories[category];
                          const itemIndex = catArray.findIndex(i => i.id === item.id);
                          catArray[itemIndex].item_type = e.target.value;
                          saveMenuToCloud(updatedMenu);
                        }}
                        style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--bg-main)', color: 'var(--text-main)', fontSize: '0.9rem', cursor: 'pointer' }}
                      >
                        <option value="none">None</option>
                        <option value="standard">Standard Item</option>
                        <option value="recipe">Recipe Based</option>
                      </select>
                    </div>
