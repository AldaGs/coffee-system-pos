...
function PinChallengeModal({ pinChallenge, setPinChallenge, challengePinAttempt, setChallengePinAttempt, handleChallengeKeyDown, challengeError, handleChallengeSubmit }) {
  ...
  // --- NEW: KEYBOARD SUPPORT FOR OVERRIDES ---
  useEffect(() => {
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 1000 }}>
      <div className={`modal-content fade-in ${challengeError ? 'shake' : ''}`} style={{ maxWidth: '350px', textAlign: 'center', background: 'var(--bg-surface)', padding: '32px', borderRadius: '16px' }}>
        ...
        <p style={{ color: 'var(--text-muted)', marginBottom: '20px' }}>Enter Manager PIN</p>
        ...
        <div style={{ 
          ...
          {challengePinAttempt.replace(/./g, '●') || <span style={{opacity: 0.2, letterSpacing: 'normal', fontSize: '1rem'}}>REQUIRED</span>}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '24px' }}>
          ...
          <button onClick={() => { setPinChallenge({ isOpen: false, title: "", onAuthorized: null }); setChallengePinAttempt(''); }} style={{ padding: '18px', fontSize: '0.9rem', background: 'rgba(231, 76, 60, 0.1)', border: 'none', borderRadius: '10px', cursor: 'pointer', color: '#e74c3c', fontWeight: 'bold' }}>CANCEL</button>
