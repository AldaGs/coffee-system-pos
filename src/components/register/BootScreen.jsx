...
function BootScreen({ logo, posSettings, loadingText }) {
  ...
  const backgroundColor = posSettings?.backgroundColor || "var(--bg-app, #f8fafc)";
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', width: '100vw', backgroundColor: backgroundColor, justifyContent: 'center', alignItems: 'center', color: 'var(--text-main)', fontFamily: 'var(--font-main, system-ui)' }}>
      ...
      {logo ? (
        <img src={logo} alt="App Logo" style={{ width: '120px', height: '120px', objectFit: 'contain', marginBottom: '24px', borderRadius: '24px', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }} />
      ) : (
      ...
      <div className="spinner" style={{ borderTopColor: 'var(--brand-color, #3498db)', width: '30px', height: '30px', borderWidth: '3px' }}></div>
