  ...
  const handleHolyGrailInstall = async (e) => {
    ...
    try {
      ...
      const schemaQuery = `
        ...
        -- it is set true automatically when an item is a Transform target. Link
        -- key is item_name (inventory.name is UNIQUE) so rows round-trip
        -- local<->cloud. Mirrors api/install.js.
        -- ==========================================
        ...
        -- Additive; stock stays owned by inventory.current_stock. Mirrors
        -- api/install.js.
        -- ==========================================
      ...
      // --- STEP C2: Turn off public sign-ups ---
      // Supabase ships "Allow new users to sign up" ON. The POS never uses it:
      // the owner's account is created just above, and devices are paired
      // through /api/add-device. Left on, anyone could create an account on
      // this project. Non-fatal -- the install is already done, so a failure
      // here is a warning, not an abort.
      try {
        const lockRes = await fetch(`/api/supabase?op=lockauth&projectRef=${selectedProject}`, {
          method: 'POST',
        });
        if (!lockRes.ok) {
          console.error('No se pudieron desactivar los registros públicos:', await lockRes.text());
        }
      } catch (lockErr) {
        console.error('No se pudieron desactivar los registros públicos:', lockErr);
      }
