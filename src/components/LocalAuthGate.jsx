...
export default function LocalAuthGate({ onAuthed }) {
  ...
  const [createStep, setCreateStep] = useState('account'); // 'account' | 'pin'
  ...
  // Validate the account fields (email + password) shared by both intents.
  const accountError = () => {
    const cleanEmail = email.trim().toLowerCase();
    if (!EMAIL_RE.test(cleanEmail)) return 'Ingresa un correo válido.';
  ...
  const handleSubmit = async (e) => {
    ...
    // --- UNLOCK: single step ---
    if (intent === 'unlock') {
      const aErr = accountError();
      if (aErr) { setError(aErr); return; }
      setBusy(true);
      try {
        const ok = await verifyLocalCredential(cleanEmail, password);
        if (!ok) { setError('Correo o contraseña incorrectos.'); setBusy(false); return; }
        onAuthed();
      } catch (err) {
    ...
    // --- CREATE, step 1: account → advance to PIN step ---
    ...
    // --- CREATE, step 2: PIN → create everything ---
    if (!/^\d{4,6}$/.test(pin)) {
  ...
  const isPinStep = isCreate && createStep === 'pin';
  const showAccountFields = !isPinStep; // unlock + create-account step

  const headerIcon = isPinStep ? 'lucide:shield' : (isCreate ? 'lucide:store' : 'lucide:lock');
  const headerTitle = isPinStep ? 'Crea tu PIN' : (isCreate ? 'Crea tu tienda local' : 'Desbloquea tu tienda');
  const headerSubtitle = isPinStep
    ? 'Elige un PIN de 4 a 6 dígitos para desbloquear la caja y el panel.'
    : (isCreate
        ? 'Tus datos se guardan solo en este dispositivo. Sin nube, sin cuentas.'
        : 'Ingresa tu correo y contraseña para continuar.');
  const submitLabel = isPinStep ? 'Empezar' : (isCreate ? 'Continuar' : 'Entrar');
  const submitIcon = busy ? 'lucide:loader-2' : (isPinStep ? 'lucide:rocket' : (isCreate ? 'lucide:arrow-right' : 'lucide:unlock'));
      ...
      <div className="fade-in" style={{ background: 'var(--bg-surface)', padding: '40px', borderRadius: '24px', width: '100%', maxWidth: '420px', boxShadow: '0 20px 50px rgba(0,0,0,0.1)', border: '1px solid var(--border)' }}>
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          {isCreate && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: '6px', marginTop: '14px' }}>
              {['account', 'pin'].map((s) => (
                <span key={s} style={{ width: '28px', height: '4px', borderRadius: '2px', background: createStep === s || (s === 'account' && isPinStep) ? '#099b46' : 'var(--border)' }} />
              ))}
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          {isPinStep && (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontWeight: 'bold', color: 'var(--text-main)', fontSize: '0.9rem' }}>PIN de administrador</label>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    required
                    disabled={busy}
                    style={{ width: '100%', padding: '12px 12px 12px 38px', borderRadius: '10px', border: '1px solid var(--border)', background: 'var(--bg-main)', color: 'var(--text-main)', fontSize: '1.3rem', letterSpacing: '0.5em', boxSizing: 'border-box' }}
                  />
                </div>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label style={{ fontWeight: 'bold', color: 'var(--text-main)', fontSize: '0.9rem' }}>Confirma tu PIN</label>
                <div style={{ position: 'relative' }}>
                  <Icon icon="lucide:shield-check" style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} />
                  <input
                    type="password"
                    inputMode="numeric"
                    autoComplete="off"
                ...
                <small style={{ color: 'var(--text-muted)' }}>Entre 4 y 6 dígitos.</small>
              </div>
            </>
          )}
          ...
          <button type="submit" disabled={busy} style={{ padding: '16px', background: '#099b46', color: 'white', border: 'none', borderRadius: '12px', cursor: busy ? 'not-allowed' : 'pointer', fontWeight: 'bold', marginTop: '6px', fontSize: '1.05rem', opacity: busy ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
            <Icon icon={submitIcon} style={{ animation: busy ? 'spin 1s linear infinite' : 'none' }} />
            <span>{busy ? 'Un momento…' : submitLabel}</span>
          </button>

          {isPinStep && !busy && (
            <button type="button" onClick={() => { setError(''); setCreateStep('account'); }} style={{ padding: '8px', background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.9rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
