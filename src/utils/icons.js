import { addIcon } from '@iconify/react';

// LUCIDE ICONS
import activity from '@iconify-icons/lucide/activity';
import alertCircle from '@iconify-icons/lucide/alert-circle';
import alertTriangle from '@iconify-icons/lucide/alert-triangle';
import arrowLeft from '@iconify-icons/lucide/arrow-left';
import award from '@iconify-icons/lucide/award';
import badgeDollarSign from '@iconify-icons/lucide/badge-dollar-sign';
import banknote from '@iconify-icons/lucide/banknote';
import barChart3 from '@iconify-icons/lucide/bar-chart-3';
import bookOpen from '@iconify-icons/lucide/book-open';
import calculator from '@iconify-icons/lucide/calculator';
import calendar from '@iconify-icons/lucide/calendar';
import check from '@iconify-icons/lucide/check';
import checkCircle from '@iconify-icons/lucide/check-circle';
import checkCircle2 from '@iconify-icons/lucide/check-circle-2';
import chefHat from '@iconify-icons/lucide/chef-hat';
import chevronsUpDown from '@iconify-icons/lucide/chevrons-up-down';
import clipboardCheck from '@iconify-icons/lucide/clipboard-check';
import clock from '@iconify-icons/lucide/clock';
import coffee from '@iconify-icons/lucide/coffee';
import creditCard from '@iconify-icons/lucide/credit-card';
import database from '@iconify-icons/lucide/database';
import databaseBackup from '@iconify-icons/lucide/database-backup';
import deleteIcon from '@iconify-icons/lucide/delete';
import download from '@iconify-icons/lucide/download';
import edit2 from '@iconify-icons/lucide/edit-2';
import edit3 from '@iconify-icons/lucide/edit-3';
import eye from '@iconify-icons/lucide/eye';
import fileQuestion from '@iconify-icons/lucide/file-question';
import flame from '@iconify-icons/lucide/flame';
import flaskConical from '@iconify-icons/lucide/flask-conical';
import folderOpen from '@iconify-icons/lucide/folder-open';
import folderPlus from '@iconify-icons/lucide/folder-plus';
import gift from '@iconify-icons/lucide/gift';
import hash from '@iconify-icons/lucide/hash';
import image from '@iconify-icons/lucide/image';
import imagePlus from '@iconify-icons/lucide/image-plus';
import key from '@iconify-icons/lucide/key';
import keyRound from '@iconify-icons/lucide/key-round';
import languages from '@iconify-icons/lucide/languages';
import layers from '@iconify-icons/lucide/layers';
import layoutDashboard from '@iconify-icons/lucide/layout-dashboard';
import layoutGrid from '@iconify-icons/lucide/layout-grid';
import layoutPanelLeft from '@iconify-icons/lucide/layout-panel-left';
import link from '@iconify-icons/lucide/link';
import link2 from '@iconify-icons/lucide/link-2';
import listOrdered from '@iconify-icons/lucide/list-ordered';
import listPlus from '@iconify-icons/lucide/list-plus';
import loader2 from '@iconify-icons/lucide/loader-2';
import lock from '@iconify-icons/lucide/lock';
import logIn from '@iconify-icons/lucide/log-in';
import logOut from '@iconify-icons/lucide/log-out';
import mail from '@iconify-icons/lucide/mail';
import mapPin from '@iconify-icons/lucide/map-pin';
import menu from '@iconify-icons/lucide/menu';
import messageSquare from '@iconify-icons/lucide/message-square';
import monitor from '@iconify-icons/lucide/monitor';
import packageCheck from '@iconify-icons/lucide/package-check';
import packagePlus from '@iconify-icons/lucide/package-plus';
import packageSearch from '@iconify-icons/lucide/package-search';
import palette from '@iconify-icons/lucide/palette';
import partyPopper from '@iconify-icons/lucide/party-popper';
import pause from '@iconify-icons/lucide/pause';
import pencilLine from '@iconify-icons/lucide/pencil-line';
import percent from '@iconify-icons/lucide/percent';
import play from '@iconify-icons/lucide/play';
import plus from '@iconify-icons/lucide/plus';
import plusCircle from '@iconify-icons/lucide/plus-circle';
import plusSquare from '@iconify-icons/lucide/plus-square';
import printer from '@iconify-icons/lucide/printer';
import receipt from '@iconify-icons/lucide/receipt';
import refreshCw from '@iconify-icons/lucide/refresh-cw';
import rocket from '@iconify-icons/lucide/rocket';
import rotateCcw from '@iconify-icons/lucide/rotate-ccw';
import save from '@iconify-icons/lucide/save';
import settings from '@iconify-icons/lucide/settings';
import settings2 from '@iconify-icons/lucide/settings-2';
import shield from '@iconify-icons/lucide/shield';
import shieldAlert from '@iconify-icons/lucide/shield-alert';
import shieldCheck from '@iconify-icons/lucide/shield-check';
import shoppingBag from '@iconify-icons/lucide/shopping-bag';
import sortAsc from '@iconify-icons/lucide/sort-asc';
import sortDesc from '@iconify-icons/lucide/sort-desc';
import sparkles from '@iconify-icons/lucide/sparkles';
import star from '@iconify-icons/lucide/star';
import store from '@iconify-icons/lucide/store';
import tag from '@iconify-icons/lucide/tag';
import tags from '@iconify-icons/lucide/tags';
import target from '@iconify-icons/lucide/target';
import terminal from '@iconify-icons/lucide/terminal';
import ticket from '@iconify-icons/lucide/ticket';
import trash2 from '@iconify-icons/lucide/trash-2';
import trendingUp from '@iconify-icons/lucide/trending-up';
import type from '@iconify-icons/lucide/type';
import undo2 from '@iconify-icons/lucide/undo-2';
import unlock from '@iconify-icons/lucide/unlock';
import upload from '@iconify-icons/lucide/upload';
import uploadCloud from '@iconify-icons/lucide/upload-cloud';
import user from '@iconify-icons/lucide/user';
import userPlus from '@iconify-icons/lucide/user-plus';
import users from '@iconify-icons/lucide/users';
import users2 from '@iconify-icons/lucide/users-2';
import x from '@iconify-icons/lucide/x';
import xCircle from '@iconify-icons/lucide/x-circle';
import zap from '@iconify-icons/lucide/zap';

// MDI ICONS
import github from '@iconify-icons/mdi/github';
import whatsapp from '@iconify-icons/mdi/whatsapp';

const icons = {
  'lucide:activity': activity,
  'lucide:alert-circle': alertCircle,
  'lucide:alert-triangle': alertTriangle,
  'lucide:arrow-left': arrowLeft,
  'lucide:award': award,
  'lucide:badge-dollar-sign': badgeDollarSign,
  'lucide:banknote': banknote,
  'lucide:bar-chart-3': barChart3,
  'lucide:book-open': bookOpen,
  'lucide:calculator': calculator,
  'lucide:calendar': calendar,
  'lucide:check': check,
  'lucide:check-circle': checkCircle,
  'lucide:check-circle-2': checkCircle2,
  'lucide:chef-hat': chefHat,
  'lucide:chevrons-up-down': chevronsUpDown,
  'lucide:clipboard-check': clipboardCheck,
  'lucide:clock': clock,
  'lucide:coffee': coffee,
  'lucide:credit-card': creditCard,
  'lucide:database': database,
  'lucide:database-backup': databaseBackup,
  'lucide:delete': deleteIcon,
  'lucide:download': download,
  'lucide:edit-2': edit2,
  'lucide:edit-3': edit3,
  'lucide:eye': eye,
  'lucide:file-question': fileQuestion,
  'lucide:flame': flame,
  'lucide:flask-conical': flaskConical,
  'lucide:folder-open': folderOpen,
  'lucide:folder-plus': folderPlus,
  'lucide:gift': gift,
  'lucide:hash': hash,
  'lucide:image': image,
  'lucide:image-plus': imagePlus,
  'lucide:key': key,
  'lucide:key-round': keyRound,
  'lucide:languages': languages,
  'lucide:layers': layers,
  'lucide:layout-dashboard': layoutDashboard,
  'lucide:layout-grid': layoutGrid,
  'lucide:layout-panel-left': layoutPanelLeft,
  'lucide:link': link,
  'lucide:link-2': link2,
  'lucide:list-ordered': listOrdered,
  'lucide:list-plus': listPlus,
  'lucide:loader-2': loader2,
  'lucide:lock': lock,
  'lucide:log-in': logIn,
  'lucide:log-out': logOut,
  'lucide:mail': mail,
  'lucide:map-pin': mapPin,
  'lucide:menu': menu,
  'lucide:message-square-text': messageSquare,
  'lucide:monitor': monitor,
  'lucide:package-check': packageCheck,
  'lucide:package-plus': packagePlus,
  'lucide:package-search': packageSearch,
  'lucide:palette': palette,
  'lucide:party-popper': partyPopper,
  'lucide:pause': pause,
  'lucide:pencil-line': pencilLine,
  'lucide:percent': percent,
  'lucide:play': play,
  'lucide:plus': plus,
  'lucide:plus-circle': plusCircle,
  'lucide:plus-square': plusSquare,
  'lucide:printer': printer,
  'lucide:receipt': receipt,
  'lucide:refresh-cw': refreshCw,
  'lucide:rocket': rocket,
  'lucide:rotate-ccw': rotateCcw,
  'lucide:save': save,
  'lucide:settings': settings,
  'lucide:settings-2': settings2,
  'lucide:shield': shield,
  'lucide:shield-alert': shieldAlert,
  'lucide:shield-check': shieldCheck,
  'lucide:shield-lock': shieldCheck,
  'lucide:shopping-bag': shoppingBag,
  'lucide:sort-asc': sortAsc,
  'lucide:sort-desc': sortDesc,
  'lucide:sparkles': sparkles,
  'lucide:star': star,
  'lucide:store': store,
  'lucide:tag': tag,
  'lucide:tags': tags,
  'lucide:target': target,
  'lucide:terminal': terminal,
  'lucide:ticket': ticket,
  'lucide:ticket-plus': ticket,
  'lucide:trash-2': trash2,
  'lucide:trending-up': trendingUp,
  'lucide:type': type,
  'lucide:undo-2': undo2,
  'lucide:unlock': unlock,
  'lucide:upload': upload,
  'lucide:upload-cloud': uploadCloud,
  'lucide:user': user,
  'lucide:user-plus': userPlus,
  'lucide:users': users,
  'lucide:users-2': users2,
  'lucide:x': x,
  'lucide:x-circle': xCircle,
  'lucide:zap': zap,
  'mdi:github': github,
  'mdi:whatsapp': whatsapp,
};

// Register icons with Iconify
Object.entries(icons).forEach(([name, data]) => {
  addIcon(name, data);
});
